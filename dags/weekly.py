...
t1 = BashOperator(
    task_id='load_data',
    bash_command='papermill {{ airflow_home }}/scripts/get_data.ipynb - -p date {{ ds }} -p period weekly > /dev/null',
    dag=dag,
)
...
t3 = BashOperator(
    task_id='report_html',
    bash_command='jupyter nbconvert {{ airflow_home }}/reports/weekly/notebooks/report_{{ ds }}.ipynb --output {{ airflow_home }}/reports/weekly/html/report_{{ ds }} --to html --no-input',
    dag=dag,
)
